...
contract PosBlockPool is Object {
    ...
    function registerNewTx(uint _poolIndex, uint _blockIndex, bytes32 _tx, bytes32 _sender, bytes32 _receiver, uint _gasUsage) internal returns (bool) {
        ...
        return PosBloc(pools_[_poolIndex].blocks_[_blockIndex]).registerTx(bytes32 _tx, bytes32 _sender, bytes32 _receiver, uint _gasUsage);
    }
    ...
    function numBlock(uint _poolIndex) internal constant returns (uint) { 
